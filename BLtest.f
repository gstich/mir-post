...
MODULE BLtest_data
  ...
  CHARACTER(LEN=30) 	:: prob_jobname = 'BLtest'
...
SUBROUTINE prob_inputs(fileName)
  ...
  NAMELIST /BLtest_vars/ Tinitial,Pinitial,del_BL,Re_BL,Pr,rcy_pt_g,init_type,restart_dir,init_stats,BL_flag,&
  ...
  READ(UNIT=inputUnit,NML=BLtest_vars)
  CLOSE(inputUnit)
...
SUBROUTINE prob_bc(rho,u,v,w,e,Y)
  ...
  INTEGER :: i,k
      ...
      IF (xnproc) THEN 
         ...
         e = e + dumT * ( .95*P_in/(gamma-one)/rho - e )                                           
      END IF  
...
SUBROUTINE prob_geom()
 ...
 DOUBLE PRECISION, DIMENSION(768,256) :: tmpx,tmpy
...
SUBROUTINE inflow_rcy_v2(rho,u,v,w,e) !!,rcy_pt_g,del_BL,Len_BL,U_in)
      ...
      !! Constants for inner/outer BL and for RR
      iBB = 0.2d0
      BB = 2.0d0
...
USE mpi
USE BLtest_data, ONLY: MAP,pmapID,rcy_pt_g,flippy
IMPLICIT NONE
...
INTEGER :: ii,rcycle,plane,side,zprocID,yprocID,recv,donor,zproc,top,bot
LOGICAL :: onMAP
...
IF(onMAP) THEN
      ...
      DO ii=istart,yproc
         ...
         IF(pdonor) THEN
            ...
            rcycle = MOD( yprocID+ii-2 , yproc ) + 1 !! goes from (1-yproc)... cycle upwards
            rproc = MAP( recv , side, zprocID, rcycle , 1)
            ...
            IF (rproc .NE. xyzcom_id) THEN
               ...
               !CALL MPI_SEND(Lplane,ay*az*nvar,MPI_DOUBLE_PRECISION,rproc,mpitag,xyzcom,mpierr)
         ...
         IF(precv) THEN
            ...
            rcycle = MOD( yproc + yprocID - (ii-1) -1 , yproc ) + 1 !! goes from (yproc-1)... cycle downwards
            sproc = MAP( donor, side, zprocID, rcycle , 1) !! This is just xyzcom_id IF your a donor
            ...
            IF (sproc .NE. xyzcom_id) THEN
               ...
               !IF(xyzcom_id==0) print*,'recv from ', sproc
            ELSE
               Lbuff = Lplane   ! Data is local so just reassign array
            END IF

            !! Keep this comment here...
            !rcycle = MOD( yprocID+ii-2 , yproc ) + 1 !! goes from (1-yproc)1
