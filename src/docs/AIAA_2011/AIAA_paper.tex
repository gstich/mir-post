...
Recently, Large Eddy Simulation (LES) has had excellent success in providing results which capture the unsteady flow physics of shock wave boundary layer interactions.\cite{Morgan:10b,Kawai:10aiaa,Touber:09,Pirozzoli:09}  Reasonable comparison to experiments at ever increasing Reynolds numbers are proving LES to be a valuable tool for engineering design and fundamental scientific research.  We wish to leverage this technique to generate a rich data set which can be interrogated at a higher level of sophistication then experimental data can afford.  Coarse mesh calculations have been completed in a relatively short amount of time and subsequent refined calculations are underway.  Data from these calculations are anticipated to be the focus of the proposed study.  We provide a brief summary of the methodology for the LES we are conducting and show preliminary results of a coarse mesh calculation which shows good qualitative agreement with the experiment.
...
The far field boundary condition is a non-reflecting characteristic boundary condition\cite{Poinsot:92} with a ``buffer'' region applied at the last few grid point which smooths out gradients and enforces the ambient conditions.  The nozzle walls are viscous and adiabatic, e.g. $\vec{u}_w=0$ and $\vec{q}_w\cdot\vec{n}_w = 0$.  The span-wise direction is periodic and the inflow values $\vec{u}_i$, $p_i$ and $\rho_i$ are given by the quasi-1d approximation.  Furthermore, it becomes computationally infeasible to resolve the boundary layer of the entire diverging section.  Rather, we begin our simulation at the throat of the nozzle and use the method of recycling-rescaling\cite{Morgan:10a,Lund:98} to continuously feed the simulation an accurately evolving turbulent boundary layer.  Figure~\ref{fig:info_3d} shows the inflow and nozzle portion of the domain and depicts the recycling-rescaling section.
