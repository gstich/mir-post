...
SUBROUTINE get_ALL(vfile,mean,flc)
  USE operators
  USE globals
  USE writeplanes_data
  IMPLICIT NONE
  DOUBLE PRECISION, DIMENSION(nx,ny,DIM), INTENT(OUT) :: mean,flc
  CHARACTER(LEN=flen), INTENT(IN) :: vfile

  DOUBLE PRECISION, DIMENSION(:,:,:,:), ALLOCATABLE :: data,dflc
  INTEGER :: k
  ...
  !! Get the fluctuating field
  ALLOCATE(dflc(nx,ny,nz,DIM))
  DO k=1,nz
     dflc(:,:,k,:) = data(:,:,k,:) - mean
  END DO
  dflc = dflc**2
