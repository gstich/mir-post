...
INTEGER, PARAMETER :: seg4r=30
INTEGER, PARAMETER :: seg4l=25
...
DOUBLE PRECISION , PARAMETER :: R4 = 1.5D0*noz_y                 ! Radius of back turn
DOUBLE PRECISION , PARAMETER :: rise = 15.18              ! Thickness of upper/lower flange at nozzle exit


!  Grid Parameters
INTEGER , PARAMETER          :: nx = 512                    ! Number of points in /xi direction
...
DOUBLE PRECISION , PARAMETER :: loci = dble(nx)*.92d0       ! Location in grid number of stretch-> iso interface
...
DOUBLE PRECISION, DIMENSION(seg4r) :: x4r,y4r
DOUBLE PRECISION, DIMENSION(seg4l) :: x4l,y4l
...
DOUBLE PRECISION :: x30,x3c,y30,y3c,dydx3,theta30,theta3f,x3f,y3f
...
!! REAlly just a straight line for the given conditions
...
DO i=1,seg4
   x4(i) = x40
   y4(i) = (rise)/dble(seg4) * dble(i) + y40
   WRITE(funit,*) x4(i),y4(i)
END DO


!! Add segment here !!
!! Bend back the vertical line
...
DO i=1,seg4r
   theta3(i) = (theta3f-theta30)/dble(seg3)*dble(i)+theta30
   x4r(i) = R4*cos(theta3(i)) + x3c
   y4r(i) = R4*sin(theta3(i)) + y3c
   WRITE(funit,*) x4r(i),y4r(i)
END DO

!! Line running back to circle
y30 = y4r(seg4r) 
y3f = y30


x30 = x4r(seg4r)
x3f = x40 - sqrt( R5**2 - (zero-y3f)**2 )

DO i=1,seg4l
   x4l(i) = (x3f-x30)/dble(seg4l) * dble(i) + x30
   y4l(i) = (y3f-y30)/dble(seg4l) * dble(i) + y30
   IF (i==seg4l) THEN    
      WRITE(funit,*) x4l(i),y4l(i),1
   ELSE
      WRITE(funit,*) x4l(i),y4l(i)
   END IF
END DO
...
x5c = x40
y5c = zero
theta50 = pi - atan( abs(y3f) / abs( x3f) ) 
theta5f = zero

DO i=1,seg5
   theta5(i) = (theta5f - theta50)/dble(seg5-1) * dble(i-1) + theta50
